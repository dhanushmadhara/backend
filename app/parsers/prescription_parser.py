...
class PrescriptionParser(BaseParser):
    ...
    def get_medicines(self):
        m = re.search(r"Address[\\s\\S]*?\\n(.*?)\\nDirections", self.text, flags=re.IGNORECASE|re.DOTALL)
        if m:
            return "\\n".join([ln.strip() for ln in m.group(1).split("\\n") if ln.strip()])
        meds = re.findall(r'^[A-Z][A-Za-z0-9\\- ]+\\s\\d+\\s?mg.*$', self.text, flags=re.MULTILINE)
        return "\\n".join(meds) if meds else None

    def get_directions(self):
        m = re.search(r"Directions[:\\s]+(.+?)Refill", self.text, flags=re.IGNORECASE|re.DOTALL)
        return m.group(1).strip() if m else None

    def get_refills(self):
        m = re.search(r"Refill[:\\s]+(\\d+)", self.text, flags=re.IGNORECASE)
